...
public class NetworkScaleChange : MonoBehaviourPun, IPunObservable
{
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        if (stream.IsWriting)
        {
            stream.SendNext(_currentLengthWeight);
            stream.SendNext(_currentWidthWeight);
            stream.SendNext(_boxCollider.size);
        }
        else
        {
            _currentLengthWeight = (float)stream.ReceiveNext();
            _currentWidthWeight = (float)stream.ReceiveNext();
            _boxCollider.size = (Vector3)stream.ReceiveNext();
            skinnedMeshRenderer.SetBlendShapeWeight(_lengthIndex, _currentLengthWeight);
            skinnedMeshRenderer.SetBlendShapeWeight(_widthIndex, _currentWidthWeight);
        }
    }
}